 ...
 * so it is race-free and never busy-waits.  GNOS spinlocks carry no saved
 * interrupt state (the kernel is serialised by the BKL instead), so there is
 * nothing to restore on either path.
 */
int drm_modeset_lock_single_interruptible(struct drm_modeset_lock *lock)
{
    uint32_t desired = 1;

    __asm__ volatile("lock xchgl %[desired], %[lock];"
                     : [lock] "+m"(lock->mutex.v), [desired] "+r"(desired)
                     :: "memory");

    if (desired != 0) {
        /* Already held: nothing was claimed. */
        return -EBUSY;
    }
