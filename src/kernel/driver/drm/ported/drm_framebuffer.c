...
int drm_mode_rmfb(struct drm_device *dev, void *data, struct drm_file *file_priv)
{
    ...
    /* Userspace passes the mode-object id (see the note in
     * drm_framebuffer_lookup): resolve through the shared object table. */
...
struct drm_framebuffer *drm_framebuffer_lookup(struct drm_device *dev, struct drm_file *file_priv, uint32_t id)
{
    ...
    /* Userspace names framebuffers by their mode-object id -- the one shared
     * id space that also holds crtcs, planes and connectors, and the space
     * ADDFB/ADDFB2 hand ids out from.  The private fb_idr below numbers from
     * its own sequence, so resolving an object id there misses for every fb
     * whose two allocations disagree (observed: ADDFB2 returned fb 22/23,
     * SETCRTC then failed with ENOENT).  Resolve through the object table
     * first; keep the legacy idr as a fallback for internal callers. */
    struct drm_mode_object *obj =
        drm_mode_object_find(dev, file_priv, id, DRM_MODE_OBJECT_FB);
    if (obj) {
        fb = container_of(obj, struct drm_framebuffer, base);
        drm_mode_object_put(obj);   /* lookup contract: no extra ref out */
        return fb;
    }
